  ...
  <div class="home">
    ...
    <section>
      <el-row class="row-bg" justify="space-between" type="flex" :gutter="20">
        ...
        <el-col :span="8">
          <div class="colBox" style="height:420px;">
    ...
    <section>
      <el-row class="row-bg" :gutter="20">
        <el-col :span="24">
          <div class="colBox">
            <span class="homeTitle">资料到报情况统计</span>
            <div class="chartsBox" id="dataArrivedChart"></div>
          </div>
        </el-col>
      </el-row>
    </section>
    ...
    <section>
      <el-row class="row-bg" :gutter="20">
        <el-col :span="24">
          <div class="colBox">
            <span class="homeTitle">数据库资料统计</span>
            <div class="chartsBox" style="padding: 20px;height:600px;overflow: auto">
              <template>
                <el-table
                  :data="tableData"
                  style="width: 100%">
    ...
    <!--<section style="background:#fff;padding-bottom:20px;">
      ...
      &lt;!&ndash; v-for="r in 5" :key="r" &ndash;&gt;
...
export default {
  ...
  created() {
    ...
    this.drawDataArrivedChart();
    this.countDatabaseTableInfo();
  },
  watch: {
    pieBox: function() {
  ...
  methods: {
    ...
    //资料到报情况
    drawDataArrivedChart() {
      findArrivedDataCount().then(res => {
        let data = res.data;
        let dataNumChart = [];
        let dataTypeChart = [];
        data.forEach(element => {
          dataNumChart.push(element.PRO);
          dataTypeChart.push(element.NAME_CN);
        });
        // 基于准备好的dom，初始化echarts实例
        var myChart = echarts.init(
          document.getElementById("dataArrivedChart")
        );
        // 绘制图表
        myChart.setOption({
          grid: {
    ...
    //数据库资料统计
    countDatabaseTableInfo() {
      this.tableData = [];
      findDataTableCount().then(res => {
        let data = res.data;
        for(let i in data){
          let obj = data[i];
          let tableObj = {
            serial: i,
            dataName: obj["CLASS_NAME"],
            dateBegin: format(obj["BEGIN_TIME"]),
            dateEnd: format(obj["END_TIME"]),
            dataNum: obj["RECORD_COUNT"],
            dataSpace: obj["DAY_TOTAL"],
          }
