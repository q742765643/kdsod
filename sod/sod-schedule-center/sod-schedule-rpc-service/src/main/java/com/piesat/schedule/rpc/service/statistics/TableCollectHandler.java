...
public class TableCollectHandler  implements BaseHandler {
        ...
        if(databaseEntities != null && databaseEntities.size()>0){
            for(DatabaseEntity databaseEntity : databaseEntities) {
                ...
                try {
                    ...
                    String databaseIp = databaseEntity.getDatabaseDefine().getDatabaseIp();
                        ...
                        if (String.valueOf(table_name).contains("1981_2010")) {
                            ...
                            try {
                                if ("Cassandra".equalsIgnoreCase(databaseType)){
                                    //获取总记录数
                                    record_count = fileCassandraSodService.queryRecordNum(databaseEntity.getSchemaName(), table_name);
                                    //最早记录的时间
                                    begin_time = fileCassandraSodService.queryMinTime(databaseEntity.getSchemaName(), table_name, "D_DATETIME");
                                    //最近记录的时间
                                    end_time = fileCassandraSodService.queryMaxTime(databaseEntity.getSchemaName(), table_name, "D_DATETIME");
                                    //获取日增量
                                    day_total = fileCassandraSodService.queryIncreCount(databaseEntity.getSchemaName(), table_name, "D_DATETIME", yesterdayZero, todayZero);
    ...
    public void queryElapsedTime(TableDataStatisticsEntity tableDataStatisticsEntity,String schema, String tableName, String timeColumnName,String beginTime,String endTime){
        try {
            String time=fileCassandraSodService.queryElapsedTime(schema,tableName,timeColumnName,beginTime,endTime);
            BigDecimal timeB=new BigDecimal(time).divide(new BigDecimal(1000),1,BigDecimal.ROUND_HALF_UP);
            tableDataStatisticsEntity.setElapsedTime(timeB);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
